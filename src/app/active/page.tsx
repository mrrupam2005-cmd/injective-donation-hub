...
export default function ActiveProposalsPage() {
  const { proposals, isLoading } = useProposal('Active');

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 border-b border-slate-800/80 pb-4">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyber-green/10 text-cyber-green border border-cyber-green/30">
          <Zap className="w-5 h-5 animate-pulse" />
        </div>
        <div>
          <h1 className="text-xl font-black text-slate-100 uppercase tracking-wider">
            Active Governance Voting
          </h1>
          <p className="text-xs text-slate-400 font-mono">
            Proposals currently open for INJ voter participation.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-12 text-center text-slate-400 font-mono text-xs">
          No active proposals currently open for voting.
        </div>
      ) : (
